...
# perturbations
#all_graphs=($n\_BA_3 $n\_ER_$er_p $n\_WS_4_0.1 $n\_kreg_3 $n\_knn_3 $n\_ERA_$er_p\_0.8 $n\_SBM_3)
all_graphs=($n\_SBM_3)
...
for graph_string in "${all_graphs[@]}"; do
    ...
    for attack in "${attacks[@]}"; do
        python scripts/perturb.py --seeds $seeds --graph_string $graph_string --budget $r --attack $attack --samples 100
    done
done
